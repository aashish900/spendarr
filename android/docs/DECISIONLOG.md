...
## 2026-07-17 — Home month ring: replace income-based fill with a user-configured budget

**Context:** The ring previously filled by outflows-vs-income (`ringProgress`), with no concept of a spending target — it just reflected how much of what was earned had been spent this month. The user asked for an actual budget concept: a monthly target the ring measures against instead, full at zero spend, draining toward zero as the budget is used, then filling **red** in the opposite direction once overspent (capped at `-budget`), with the budget either constant forever or re-entered every month.

**Decision:** Replaced the ring's basis entirely — confirmed with the user via `AskUserQuestion` rather than adding a second income-based view alongside it (Income/Expense stat row is untouched; only the ring's `progress`/`amountText`/`descriptor` inputs changed). `MonthRing.progress` became a signed value in `[-1, 1]`: positive fills gold clockwise (within budget), negative fills red counter-clockwise (overspent), `0` when no budget is configured (blank ring). The budget-setup/re-entry dialog (`BudgetPromptDialog`) is a **blocking** modal (`canPop: false`, `barrierDismissible: false`) on first run and on the monthly re-prompt — `0` is accepted as a valid budget, but the dialog can't be dismissed without saving *something* — also confirmed via `AskUserQuestion` over a dismissible-defaults-to-0 alternative.

**Why:** A budget the user explicitly sets is a more actionable "am I on track" signal than one derived from income booked so far (which is noisy — income often lands as one lump sum, not evenly through the month). Blocking (rather than dismissible) ensures the ring is never silently measuring against a stale or default budget the user never actually saw.

**Alternatives considered:** Keeping the income-based ring as a fallback or toggle alongside the budget view — rejected, adds UI complexity (a mode switch) for a case the user didn't ask for; storing a distinct budget value per historical month (so browsing past months shows *that* month's budget) — not implemented; past months reuse the current budget value against that month's own outflow, same as today's month-browsing behavior for every other Home figure, and no requirement was raised for historical budget tracking.

## 2026-07-17 — Budget setup: one write path for first-run, monthly re-prompt, and Settings edits

**Context:** The budget needed to support two modes — "same every month" (set once) vs. "ask me every month" (re-prompt at the start of each calendar month) — plus manual editing from Settings at any time, all landing on the same underlying `monthly_budget_cents` / `budget_mode` state.

**Decision:** A single `ProfileNotifier.saveBudget({cents, mode, now})` write path is used by all three entry points; it always stamps `budgetSetForMonth` to the month it's called in. `needsBudgetPrompt` compares that stamp against the real current month (`localDayTickProvider`, not the `nowProvider` test-override used for the Home greeting, and not `effectiveHomeMonthProvider`, which only reflects month-*browsing*) — so `monthly` mode re-prompts once a new calendar month starts, `constant` mode never re-prompts after its first save, and editing from Settings in either mode counts as "set for this month" (no immediate re-prompt right after an edit).

**Why:** One write path avoids three slightly-different persistence code paths drifting out of sync; stamping the month on every save (including Settings edits) means a user editing their budget mid-month doesn't immediately see the monthly re-prompt again on the next Home load.

**Alternatives considered:** Storing budget history per month (a table keyed by `yyyy-MM`) instead of a single "current" value + a stamp of when it was last set — rejected as unnecessary complexity; nothing in the ask requires querying a past month's budget, only "was *this* month's budget already set."
