...
## 2026-07-19 — Summary card simplified to Income/Expenses; category pickers now 3-column grids

- `lib/widgets/summary_chips.dart` — dropped the Balance column; `SummaryChips` now shows only Income and Expenses. Expenses folds in investments (same outflow figure the ring is measured against), so investment amounts are represented there instead of a separate column.
- `lib/screens/home_screen.dart` — `SummaryChips` call site updated: `expenseCents: outflowCents` (expense + investment), `balanceCents` argument removed.
- `lib/widgets/category_picker_tile.dart` (new) — shared bordered icon+label tile for category-picker bottom sheets, replacing the single-column `ListTile` list.
- `lib/screens/add_txn_screen.dart`, `lib/screens/add_recurring_screen.dart` — `_pickCategory`'s sheet body changed from `ListView`+`ListTile` to `GridView.count(crossAxisCount: 3)` + `CategoryPickerTile`, same 3-column layout as the Categories screen's own grid. Add-transaction's "＋ New category" entry is now a grid tile too (same label text, so no behavior change for existing finders).
- Gates: `flutter analyze` clean; `flutter test` 176/176 green. Updated assertions in `home_screen_test.dart` and `add_txn_flow_test.dart` for the removed Balance chip and Expenses-now-includes-investments math; updated `add_txn_kind_filter_test.dart`'s category-sheet tests to target `CategoryPickerTile`/`GridView` instead of `ListTile`/`ListView`.
