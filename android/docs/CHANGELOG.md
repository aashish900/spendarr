...
## 2026-07-17 — Budget-based month ring, budget setup/monthly re-prompt, Recurring-chip income exclusion

- `lib/screens/home_screen.dart` — Home's Recurring summary chip previously summed *every* active recurring rule regardless of kind; an income rule (e.g. salary) inflated the figure. Now filters to `kind != income` (expense + investment only). Also replaced the income-based ring computation (`ringProgress`, income − outflow) with a budget-based one (`budgetRingProgress`, budget − outflow): the ring shows money left against the user's configured budget, not income; with no budget configured it's blank and the centre shows total spend ("spent"). `HomeScreen` converted from `ConsumerWidget` to `ConsumerStatefulWidget` to track (via a plain `_promptScheduled` bool) whether the budget-setup/re-prompt dialog has already been scheduled this widget lifetime, so it doesn't re-fire every rebuild.
- `lib/providers/profile.dart` — new `BudgetMode` enum (`constant`/`monthly`), new `Profile.budgetMode`/`budgetSetForMonth` fields (stored as two new `sync_meta` keys, same local-only/no-outbox pattern as the existing budget/name fields), new `ProfileNotifier.saveBudget()` (single write path for first-run setup, the monthly re-prompt, and Settings edits — always stamps `budgetSetForMonth` to the current month), and a pure `needsBudgetPrompt(Profile, DateTime)` helper.
- `lib/providers/summary.dart` — new `budgetRingProgress(budgetCents, outflowCents)`: `0` with no budget, `1` at zero spend, drains toward `0` as spend approaches the budget, then negative (clamped at `-1`) once overspent. The old `ringProgress` is left in place (no longer called from `HomeScreen`) rather than deleted in this pass.
- `lib/widgets/month_ring.dart` — `MonthRing.progress` is now signed (`[-1, 1]`): negative values draw a red arc counter-clockwise from the same top start point (`Canvas.drawArc` accepts a negative sweep angle directly, so no new geometry was needed), reusing the existing glow/end-cap-dot treatment with `kExpenseRed` in place of the gold gradient.
- `lib/widgets/budget_prompt_dialog.dart` (new) — one dialog for all three budget entry points (first-run setup, the monthly re-prompt, Settings edit), parameterized by `canCancel` (blocking vs. dismissible) and `showModeChoice` (mode picker shown for first-run/Settings, hidden for the monthly re-prompt since the mode is already fixed). Uses the existing `FieldCard`/`SectionLabel` and generic `PillSelector<BudgetMode>`.
- `lib/screens/settings_screen.dart` — converted to `ConsumerWidget`; added a "BUDGET" section showing the current amount + mode, tappable to open the edit dialog (`canCancel: true`).
- Gates: `flutter analyze` clean; `flutter test` 195/195 green. Existing Home ring tests rewritten for the new budget-based semantics (amount/progress math, "spent" vs "overspent" vs "left to spend"/"left over"); the shared `_pump` test helper in `home_screen_test.dart` now defaults to seeding a "budget already configured this month" state so the new blocking dialog doesn't swallow unrelated tests' interactions (opt out via `seedBudget: false` to test the prompt itself). The same seeding was needed in `add_txn_flow_test.dart`, `add_txn_inline_category_test.dart`, `add_txn_kind_filter_test.dart`, and `app_shell_test.dart`, which also boot the full app at `/home`.
